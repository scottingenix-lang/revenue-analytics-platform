...
export default async function ExecutiveCockpitPage() {
  ...
  const [
    ...
    supabase.from('sls_opportunities').select('arr, stage, deal_age_days').in('stage', ['6', 'Closed Lost']).gte('close_date', trailing12),
    ...
    supabase.from('fin_spend_monthly').select('amount').gte('fiscal_month', quarterStart(-1)).lt('fiscal_month', quarterStart(0)),
    supabase.from('fin_spend_monthly').select('amount').gte('fiscal_month', quarterStart(-2)).lt('fiscal_month', quarterStart(-1)),
    ...
    supabase.from('sub_arr_movements').select('arr_delta').gte('effective_date', quarterStart(-1)).lt('effective_date', quarterStart(0)),
  ...
  const cycleData = won.filter((o) => o.deal_age_days != null)
  const avg_cycle = cycleData.length > 0 ? cycleData.reduce((s, o) => s + Number(o.deal_age_days), 0) / cycleData.length : 0
  ...
  const sm_spend = (spendCurrentData.data ?? []).reduce((s, r) => s + Number(r.amount), 0)
  ...
  const net_new_arr = (netNewData.data ?? []).reduce((s, r) => s + Number(r.arr_delta), 0)
  ...
  const tiles: KpiTile[] = [
    ...
    { label: 'CAC Payback', value: fmtMonths(cac_payback), sub: 'Blended, prior quarter' },
    ...
    { label: 'Magic Number', value: magic_number.toFixed(2), sub: 'Net new ARR efficiency' },
